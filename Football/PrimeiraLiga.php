...
<head>
    ...
    <title>Résultats de la Primeira Liga</title>
...
<body>
    <h1>Primeira Liga</h1>
    <div id="standings">
        <!-- Tableau des résultats de la Primeira Liga -->
        <table border="1">
            <tr>
                <th>Club</th>
                <th></th>
    ...
    <script>
        ...
        const apiUrl = '../FootProxy.php?url=<?php echo urlencode($PPLapiUrl); ?>&key=<?php echo urlencode($apiKey); ?>';
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                data.standings[0].table.forEach((team, index) => {
                    tableHtml += `<tr>`;
                    if (index < 1) {
                        // Ajoutez une classe à la première cellule uniquement pour les trois premières lignes
                        tableHtml += `<td class="LDC-Group">${team.position}</td>`;
                    } else if (index === 1) {
                        // Ajoutez une classe à la quatrième cellule
                        tableHtml += `<td class="LDC-Qualif">${team.position}</td>`;
                    } else if (index === 2) {
                        // Ajoutez une classe à la cinquième cellule
                        tableHtml += `<td class="EL-Group">${team.position}</td>`;
                    } else if (index === 3) {
                        // Ajoutez une classe à la cinquième cellule
                        tableHtml += `<td class="ECL-Qualif">${team.position}</td>`;
                    } else if (index === 15) {
                        // Ajoutez une classe à la cinquième cellule
                        tableHtml += `<td class="Barrages">${team.position}</td>`;
                    } else if (index >= data.standings[0].table.length - 2) {
                        // Les 3 derniers en Relegation
                        tableHtml += `<td class="Relegation">${team.position}</td>`;
                    } else {
                        tableHtml += `<td>${team.position}</td>`;
                    }
                    tableHtml += `<td class="name"><img src="${team.team.crest}" alt="${team.team.name}" style="max-width: 40px; margin-right: 10px;">${team.team.name}</td>`;
    ...
    <div class="legende">
        ...
        <div>
            <div class="L-LDC-Qualif" style="display: inline-block;">‎</div>
            <span style="display: inline-block; vertical-align: top;">LDC-Qualif</span>
        </div>
        <div>
        ...
        <div>
            <div class="L-ECL-Qualif" style="display: inline-block;">‎</div>
            <span style="display: inline-block; vertical-align: top;">ECL-Qualif</span>
        </div>
        <div>
            <div class="L-Barrages" style="display: inline-block;">‎</div>
            <span style="display: inline-block; vertical-align: top;">Barrages</span>
        </div>
        <div>
